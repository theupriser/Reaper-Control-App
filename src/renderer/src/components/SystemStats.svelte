<script lang="ts">
  ...
  import { projectId, midiActivity } from '../stores';
  ...
  function handleSystemStats(data: any): void {
    ...
    // Estimate Electron frontend memory usage using performance API if available
    if (window.performance && window.performance.memory) {
      combinedMemoryUsage.electron = window.performance.memory.usedJSHeapSize;
    } else {
      // If performance API is not available, estimate based on backend memory
      combinedMemoryUsage.electron = Math.round(stats.memory.used * 0.2); // 20% of backend as an estimate
    }

    // Calculate total combined memory usage
    combinedMemoryUsage.total = combinedMemoryUsage.nodeBackend + combinedMemoryUsage.electron;
  ...
  onMount(() => {
    logger.log('SystemStats component mounted');

    // Listen for system stats updates from main process
    if (window.electronAPI) {
      window.electronAPI.onSystemStats(handleSystemStats);
    }
  ...
  onDestroy(() => {
    logger.log('SystemStats component destroyed');

    // Remove event listeners
    if (window.electronAPI) {
      window.electronAPI.removeAllListeners('system-stats');
    }
        ...
        <div class="stats-section">
          ...
          <div class="stats-details">
            <div class="stat-item">
              ...
              <span class="stat-value electron-status">
                Running in Electron
              </span>
            </div>
            {#if $projectId}
