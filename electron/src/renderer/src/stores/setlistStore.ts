...
import { regions } from './regionStore';
import { updatePartialPlaybackState } from './playbackStore';
 ...
 * The backend will handle selecting the first region in the setlist and pausing playback
...
export async function setSelectedSetlist(setlistId: string | null): Promise<void> {
  try {
    ...
    if (setlistId) {
      const setlist = await fetchSetlist(setlistId);
      // The backend handles selecting the first region and pausing playback
    } else {
      currentSetlist.set(null);
    }
  } catch (err) {
